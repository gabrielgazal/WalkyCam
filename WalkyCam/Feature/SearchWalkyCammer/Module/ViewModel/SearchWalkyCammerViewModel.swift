...
final class SearchWalkyCammerViewModel: SearchWalkyCammerViewModelProtocol {
    ...
    @MainActor func getUserRegion() {
        ...
        geocoder.geocodeAddressString(locationText) { (placemarks, error) in
            ...
            Task {
                await self.getWalkyCammersOnLocation(coordinates: location) {
    ...
    @MainActor private func getWalkyCammersOnLocation(coordinates: CLLocationCoordinate2D, completion: @escaping () -> Void) async {
        walkyCammers = .loading
        do {
            let cammers = try await interactor.getCammersOnLocation(location: coordinates)
    ...
    @MainActor func updateUserRegionGeocoder() {
        guard let location = mapView?.mapboxMap.cameraState.center else { return }
        Task {
            await getWalkyCammersOnLocation(coordinates: location) {
                self.mapView?.camera.ease(
                    to: CameraOptions(
                        center: location,
                        bearing: 0,
